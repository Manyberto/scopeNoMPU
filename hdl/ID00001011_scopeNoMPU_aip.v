`define ID00001011_MEM_IN 2

//`define ID00001011_MEM_OUT 2

`define ID00001011_CONF_REG 1

// `define ID00001011_STREAMING_IN
// `define ID00001011_STREAMING_OUT

module ID00001011_scopeNoMPU_aip 
(
  clk,
  rst,
  en,
...
input wire [MEM_ADDR_MAX_WIDTH-1:0] rdAddrMemIn_1;
  ...
  input wire[STATUS_WIDTH-1:0] statusIPcore;

  output startIPcore;

  ID00001011_scopeNoMPU_aipModules AIP
  (
    .clk (clk),
    .rst (rst),
    .en (en),
    ...
    .statusIPcore({1'd0, statusIPcore[STATUS_WIDTH-1:1]}),

    .intIPCore({7'b0, statusIPcore[0]}),
...
module ID00001011_scopeNoMPU_aipModules
(
  clk,
  rst,
  en,
  ...
  //--- IP-core ---//
  `ifdef ID00001011_MEM_IN
  rdDataMemIn,
  rdAddrMemIn,
  `elsif ID00001011_STREAMING_IN
  rdDataMemIn,
  rdAddrMemIn,
  `endif

  `ifdef ID00001011_MEM_OUT
  wrDataMemOut,
  wrAddrMemOut,
  wrEnMemOut,
  `elsif ID00001011_STREAMING_OUT
  wrDataMemOut,
  wrEnMemOut,
  `endif

  `ifdef ID00001011_CONF_REG
  rdDataConfigReg,
  `endif

  `ifdef ID00001011_STREAMING_IN
  dataInStreaming,
  startStreaming,
  wrInStreaming,
  `endif

  `ifdef ID00001011_STREAMING_OUT
  dataOutStreaming,
  doneStreaming,
  wrOutStreaming,
  `endif
  ...
  `ifdef ID00001011_MEM_IN
  localparam [(`ID00001011_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] MEM_IN_ADDR_WIDTH = { 16'd12, 16'd12 };

  localparam [((`ID00001011_MEM_IN*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_IN = { 5'b00011, 5'b00010, 5'b00001, 5'b00000 };
  `endif

  `ifdef ID00001011_MEM_OUT
  localparam [(`ID00001011_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] MEM_OUT_ADDR_WIDTH = { 16'd9 };

  localparam [((`ID00001011_MEM_OUT*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_OUT = { 5'b00111, 5'b00110 };
  `endif

  `ifdef ID00001011_CONF_REG
  localparam [(`ID00001011_CONF_REG*CONF_REG_ADDR_MAX_WIDTH)-1:0] CONF_REG_ADDR_WIDTH = { 3'd4 };
  ...
  `ifdef ID00001011_MEM_IN
  output [(`ID00001011_MEM_IN*DATA_WIDTH)-1:0] rdDataMemIn;
  input wire [(`ID00001011_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemIn;

  wire [(`ID00001011_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemIn;
  wire [`ID00001011_MEM_IN-1:0] wrEnMemIn;

  wire [DATA_WIDTH-1:0] rdDataMemIn0;
  `elsif ID00001011_STREAMING_IN
  output [DATA_WIDTH-1:0] rdDataMemIn;
  input wire [MEM_ADDR_MAX_WIDTH-1:0] rdAddrMemIn;
  `endif // ID00001011_MEM_IN

  `ifdef ID00001011_MEM_OUT
  input [(`ID00001011_MEM_OUT*DATA_WIDTH)-1:0] wrDataMemOut;
  input [(`ID00001011_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemOut;
  input [`ID00001011_MEM_OUT-1:0] wrEnMemOut;

  wire [(`ID00001011_MEM_OUT*DATA_WIDTH)-1:0] rdDataMemOut;
  wire [(`ID00001011_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemOut;
  `elsif ID00001011_STREAMING_OUT
  input [DATA_WIDTH-1:0] wrDataMemOut;
  input [0:0] wrEnMemOut;
  `endif // ID00001011_MEM_OUT
  ...
  `ifdef ID00001011_MEM_IN
  generate
      for(i=0; i<`ID00001011_MEM_IN; i = i + 1) begin :  MEMIN
          if (0 == i)
  ...
  `ifdef ID00001011_MEM_OUT
  generate
      for(i=0; i<`ID00001011_MEM_OUT; i = i+1) begin : MEMOUT
          simple_dual_port_ram_single_clk
          #(
              .DATA_WIDTH (DATA_WIDTH),
...
module ID00001011_scopeNoMPU_aipCtrl
(
  clk,
  rst,
  en,
  ...
  `ifdef ID00001011_MEM_IN
  parameter [((`ID00001011_MEM_IN*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_IN = {5'b00011, 5'b00010, 5'b00001, 5'b00000};
  `endif

  `ifdef ID00001011_MEM_OUT
  parameter [((`ID00001011_MEM_OUT*2)*CONFIG_WIDTH)-1:0] CONFIG_MEM_OUT = {5'b00101, 5'b00100};
  `endif
  ...
  `ifdef ID00001011_MEM_IN
  output wire [(`ID00001011_MEM_IN*MEM_ADDR_MAX_WIDTH)-1:0] wrAddrMemIn;
  output wire [`ID00001011_MEM_IN-1:0] wrEnMemIn;

  reg [MEM_ADDR_MAX_WIDTH-1:0] regWrAddrMemIn [0:`ID00001011_MEM_IN];
  `endif

  `ifdef ID00001011_MEM_OUT
  output wire [(`ID00001011_MEM_OUT*MEM_ADDR_MAX_WIDTH)-1:0] rdAddrMemOut;

  reg [MEM_ADDR_MAX_WIDTH-1:0] regRdAddrMemOut [0:`ID00001011_MEM_OUT];
  `endif
  ...
  `ifdef ID00001011_MEM_IN
  generate
      for (indexEn=0; indexEn<`ID00001011_MEM_IN; indexEn= indexEn + 1) begin : WR_EN_MEMIN
          assign wrEnMemIn[indexEn] = writeAIP & (configAIP == CONFIG_MEM_IN[CONFIG_WIDTH*(indexEn*2)+:CONFIG_WIDTH]);
      end // for ID00001011_MEM_IN
   endgenerate
  `endif
  ...
  always @(posedge clk or negedge rst) begin
    if (!rst) begin
      `ifdef ID00001011_MEM_IN
      for (i=0; i<`ID00001011_MEM_IN; i= i + 1) begin
        regWrAddrMemIn[i] <= 'd0;
      end
      `endif

      `ifdef ID00001011_MEM_OUT
      for (i=0; i<`ID00001011_MEM_OUT; i= i + 1) begin
  ...
  `ifdef ID00001011_MEM_IN
  generate
    for (indexAddr=0; indexAddr<(`ID00001011_MEM_IN); indexAddr=indexAddr+1) begin: MEM_IN_ADDR
  ...
  `ifdef ID00001011_MEM_OUT
  generate
    for (indexAddr=0; indexAddr<(`ID00001011_MEM_OUT); indexAddr=indexAddr+1) begin: MEM_OUT_ADDR
